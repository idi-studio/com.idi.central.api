...
namespace IDI.Central.Domain.Modules.Identity.Handlers
{
    public class InitializeCommandHandler : ICommandHandler<InitializeCommand>
    {
        ...
        {
            bool initialized = this.ModuleRepository.Exist(e => e.Code == command.SeedData.Modules.Identity.Code);

            if (initialized)
                return new Result { Status = ResultStatus.Success, Message = "系统数据已初始化!" };

            this.ModuleRepository.Add(command.SeedData.Modules.Identity);
